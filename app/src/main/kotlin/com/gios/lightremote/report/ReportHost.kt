...
fun ReportOverlay() {
    ...
    // sheet over whatever you were doing, and the only way out was to dismiss it. On a remote
    // that is worse than elsewhere — you shake the phone by waving it at the television.
    ...
    // being asked about a dropped connection on top of a crash report is how people turn this off.
    ...
    pending?.takeIf { !sheetOpen }?.let { why ->
        ...
        ReportChip(
            ...
            onExpire = {
                // Silence is "not now", not "no". The crash log stays on disk so the next launch
                // can offer it again; only the in-memory failure is dropped, and Trouble will not
                // re-raise the same one for an hour.
                pending = null
